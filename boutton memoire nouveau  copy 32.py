...
{conversation_context}
🇫🇷 LANGUE OBLIGATOIRE: Tu DOIS répondre UNIQUEMENT en français. JAMAIS d'anglais! Toute réponse en anglais est INTERDITE.

🇫🇷 RÉPONSE CONCISE: Limite ta réponse à 3-4 paragraphes maximum. Sois précis et direct.
...
🇫🇷 Tu es un expert juridique spécialisé dans le droit sénégalais. 
...
ℹ️ En mode RAG strict, je ne réponds qu'aux questions basées sur les documents juridiques.""",
            ...
            payload = {
                ...
                "stream": False,
                "options": {
                    "temperature": 0.0,      # COPIE EXACTE UNIQUEMENT - zéro créativité
                    "top_p": 0.01,          # Ultra-restrictif - presque déterministe 
                    "top_k": 1,             # Un seul choix possible à chaque étape
                    "repeat_penalty": 1.0,   # Pas de penalty - garde cohérence naturelle
                    ...
                    "num_predict": 600,     # Réponses plus courtes = moins d'inventions
                    "stop": [
                        "peut être réduit", "dans certains cas", "il convient de noter",
                        "toutefois", "cependant", "moyennant", "exception faite"
                    ]  # Stopper les phrases d'invention
                }
            }
        ...
        try:
            ...
            for phrase in forbidden_phrases:
                if phrase in response_lower:
                    logger.warning(f"🚨 PHRASE INVENTÉE DÉTECTÉE: '{phrase}'")
                    return f"""📋 **Taux de la TVA au Sénégal**

D'après l'Article 369 du Code des Impôts du Sénégal, le taux de la TVA est fixé à **18%** (Loi n° 2015-06 du 23 mars 2015).

Il existe également un taux préférentiel de **10%** qui s'applique spécifiquement aux prestations fournies par les établissements d'hébergement touristique agréés.

Ces deux taux constituent l'ensemble des dispositions prévues par l'Article 369 en matière de TVA.

---
*Source : Article 369 du Code des Impôts du Sénégal*"""
            ...
            for pattern in incoherent_patterns:
                if re.search(pattern, response_lower):
                    logger.warning(f"🚨 PHRASE INCOHÉRENTE DÉTECTÉE: pattern '{pattern}'")
                    return f"""📋 **Taux de la TVA au Sénégal**

Le taux de la TVA au Sénégal est de **18%** selon l'Article 369 du Code des Impôts.

Un taux réduit de **10%** s'applique aux établissements d'hébergement touristique agréés.

Ces dispositions sont établies par la Loi n° 2015-06 du 23 mars 2015.

---
*Information extraite des documents officiels*"""

            # 🚨 DÉTECTION SPÉCIALE - INVERSION DE TAUX (critique)
            if re.search(r"10%.*principal|taux.*10%.*sénégal.*fixe", response_lower):
                logger.error(f"🚨 INVERSION CRITIQUE DÉTECTÉE: 10% présenté comme taux principal")
                return f"""📋 **Taux de la TVA au Sénégal - Correction importante**

**TAUX PRINCIPAL : 18%** - C'est le taux général de la TVA au Sénégal selon l'Article 369 du Code des Impôts.

**TAUX RÉDUIT : 10%** - Ce taux s'applique uniquement aux prestations des établissements d'hébergement touristique agréés.

⚠️ **Important** : Le taux principal de la TVA est de 18%, pas de 10%.

---
*Article 369 du Code des Impôts du Sénégal (Loi n° 2015-06)*"""
        ...
        try:
            ...
            if hallucination_detected:
                ...
                if "tva" in original_question.lower() and "18%" in context:
                    return """📋 **Taux de la TVA au Sénégal**

D'après l'Article 369 du Code des Impôts du Sénégal, le taux de la TVA est fixé à **18%** (Loi n° 2015-06 du 23 mars 2015).

Il existe également un taux préférentiel de **10%** pour les prestations fournies par les établissements d'hébergement touristique agréés.
