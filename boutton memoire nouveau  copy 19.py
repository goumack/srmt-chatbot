...
Réponds UNIQUEMENT avec les 5 reformulations (pas d'explication):"""
    ...
    def is_fiscal_related_question(self, message: str) -> bool:
        """Utilise le modèle Mistral pour déterminer si la question est liée aux domaines indexés"""
        try:
            prompt = f"""🇫🇷 LANGUE OBLIGATOIRE: Tu DOIS répondre UNIQUEMENT en français.

Détermine si la question suivante est liée aux domaines suivants:
- Fiscalité, impôts, douanes, réglementation fiscale sénégalaise
- Économie, finances publiques, budget de l'État
- Secteurs économiques (chimique, agroalimentaire, etc.)
- Prévisions économiques et croissance sectorielle
- Investissements et politique économique
- Dette publique et gestion financière

Question: "{message}"

Réponds uniquement par 'OUI' si la question concerne UN OU PLUSIEURS de ces domaines indexés.
Réponds uniquement par 'NON' si la question ne concerne aucun de ces domaines (sport, technologie générale, cuisine personnelle, etc.).
IMPORTANT: Réponds UNIQUEMENT par 'OUI' ou 'NON' en français."""
            ...
            if response.status_code == 200:
                ...
                is_relevant = result == 'OUI'
                logger.info(f"🔍 Classification par Mistral: {message[:50]}... -> {'ACCEPTÉE' if is_relevant else 'REJETÉE'}")
                return is_relevant
            else:
            ...
            # En cas d'erreur, considérer que c'est une question valide par défaut
            return True
...
5. Si tu vois "35 milliards FCFA" dans le texte, dis "35 milliards FCFA" - ne dis PAS que ce n'est pas mentionné
...
- Cite-la TEXTUELLEMENT avec références précises
- Si l'information DIRECTE n'est pas dans le texte, alors RÉSUME ce que contiennent les articles/sections trouvés en expliquant leurs dispositions principales
...
- [Document/Article X] : [résumé du contenu avec chiffres exacts]
- [Document/Article Y] : [résumé du contenu avec montants précis]
Ces dispositions encadrent les aspects connexes de [domaine général]."

Réponds maintenant en français uniquement et en appliquant CES RÈGLES STRICTEMENT:"""
                else:
                    return {
...
""")
                ...
                if not self.is_fiscal_related_question(message):
                    return {
                        "response": f"""⚠️ QUESTION HORS DOMAINE
...
ℹ️ Seules les questions ayant des réponses dans les documents indexés sont traitées.""",
                    ...
                    return {
                        "response": f"""🔍 AUCUNE INFORMATION TROUVÉE
