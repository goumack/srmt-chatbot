...
=== DOCUMENTS JURIDIQUES SÉNÉGALAIS FOURNIS ===
Source: {code_source.upper()}

{context}

=== INSTRUCTIONS CRITIQUES - MODE RAG ULTRA-STRICT ===

🎯 MISSION: Extraire et citer EXACTEMENT les informations des documents sans AUCUNE modification

📋 MÉTHODOLOGIE STRICTE EN 4 ÉTAPES:

ÉTAPE 1 - COMPRENDRE LA QUESTION
• Identifie le SUJET EXACT recherché (ex: "taux de la TVA", "taux de la taxe sur les activités financières")
• Repère les MOTS-CLÉS précis (TVA, TAF, douane, impôt, procédure, etc.)
• Détermine si l'utilisateur cherche UN taux spécifique ou PLUSIEURS informations

ÉTAPE 2 - CHERCHER DANS TOUS LES DOCUMENTS FOURNIS
• Lis TOUS les passages fournis ci-dessus
• Pour CHAQUE passage, vérifie s'il répond à la question
• Ne saute AUCUN document pertinent
• Identifie TOUS les articles qui parlent du sujet demandé

ÉTAPE 3 - VALIDER LA PERTINENCE DE CHAQUE DOCUMENT
• Pour CHAQUE document trouvé, vérifie qu'il parle EXACTEMENT du sujet demandé
• NE CONFONDS PAS: "taux de la TVA" ≠ "taux de la taxe sur les activités financières" ≠ "taux de contribution forfaitaire"
• Si le document parle d'un AUTRE taux ou d'un AUTRE sujet, NE L'INCLUS PAS dans la réponse

ÉTAPE 4 - FORMAT DE RÉPONSE OBLIGATOIRE

⚠️ RÈGLE ABSOLUE: TU DOIS LISTER **TOUTES** LES RÉFÉRENCES TROUVÉES, **PAS DE RÉSUMÉ**

Pour CHAQUE document pertinent trouvé, affiche:

📄 RÉFÉRENCE [numéro]
Source: [Nom exact du fichier]
Article: [Numéro exact de l'article]
Localisation: [Page et lignes si disponibles]

📖 TEXTE EXACT DU DOCUMENT:
"[COPIE EXACTE du texte de l'article - NE MODIFIE RIEN, NE RÉSUME PAS]"

🔍 RÉPONSE À LA QUESTION:
[Extrait la réponse précise à la question posée basée sur ce texte]

[SÉPARATEUR]
---

Répète ce format pour CHAQUE document pertinent (RÉFÉRENCE 1, RÉFÉRENCE 2, RÉFÉRENCE 3, etc.)

⚠️ RÈGLES ABSOLUES - INTERDICTIONS STRICTES:
❌ NE RÉSUME JAMAIS plusieurs documents en une seule réponse
❌ NE CITE PAS un taux différent de celui demandé (ex: si on demande TVA, ne cite pas TAF)
❌ NE MODIFIE PAS les chiffres du document (17% doit rester 17%, pas 6% ou autre)
❌ NE GÉNÈRE AUCUNE information qui n'est pas dans les documents
❌ NE MÉLANGE PAS différents types de taux ou d'impôts
✅ CITE le texte EXACTEMENT comme il apparaît dans le document
✅ LISTE toutes les références pertinentes séparément
✅ SI plusieurs articles parlent du même sujet avec des taux différents, LISTE-LES TOUS

=== EXEMPLES ===

Exemple 1 - Question: "Quel est le taux de la taxe sur les activités financières?"

📄 RÉFÉRENCE 1
Source: Senegal-Code-des-impot.pdf
Article: Article 404
Localisation: Page 201, Section II

📖 TEXTE EXACT DU DOCUMENT:
"Article 404. Le taux de la taxe sur les activités financières est de 17%. Ce taux est réduit à 7% pour les intérêts, commissions et frais perçus à l'occasion de toutes les opérations finançant les ventes à l'exportation."

🔍 RÉPONSE À LA QUESTION:
Le taux de la taxe sur les activités financières est de 17% (taux général) ou 7% (pour les opérations d'exportation).

---

Exemple 2 - Si AUCUN document ne répond:
"Les documents fournis ne contiennent pas d'information sur [sujet demandé]."

=== COMMENCE TA RÉPONSE ICI ==="""
                else:
                    return {
