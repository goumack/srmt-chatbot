...
# RECHERCHE EN MODE RAG STRICT
            logger.info(f"� Recherche documentaire pour: '{message[:50]}...'")
            
            # Étape 1: Recherche principale avec la question complète
            search_result = self.search_context_with_references(message, limit=3)
            context = search_result.get("context", "")
            references = search_result.get("references", [])
            
            # Étape 2: Si aucun résultat, essai avec mots-clés extraits
            if not context:
                ...
                if keywords:
                    ...
                    search_result2 = self.search_context_with_references(keyword_query, limit=3)
            ...
            if context and context.strip():
                ...
                if keyword_found or any(keyword in context_lower for keyword in ["impot", "tva", "douane", "fiscal", "cgi", "dgi", "senegal", "sénégal", "article"]):
                    ...
                    prompt = f"""Tu es SRMT-DOCUMIND, assistant IA en mode RAG strict pour la fiscalité sénégalaise.
...
=== INSTRUCTIONS MODE RAG STRICT ===
1. UTILISE EXCLUSIVEMENT les informations des documents ci-dessus
2. NE GÉNÈRE AUCUNE information qui n'est pas dans les documents fournis
3. CITE SYSTÉMATIQUEMENT les articles et références légales exactes
4. AVERTIS clairement si une information n'est pas dans les documents
5. Format de réponse à utiliser:
   • RÉFÉRENCE: [Article X du {code_source}]
   • TEXTE EXACT: [Citation directe]
   • EXPLICATION: [Clarification simple]

IMPORTANT: Tu réponds UNIQUEMENT avec les informations des documents fournis.
NE JAMAIS inventer d'information ni utiliser tes connaissances générales.

=== RÉPONSE BASÉE EXCLUSIVEMENT SUR LES DOCUMENTS ==="""
                else:
                    return {
